...
names(test)=c('Beta1','Beta2')
test$Iteration=1:10000
test=melt(test,id.vars = 3)
names(test)=c('Iteration','Variable','Value')
qplot(Iteration,Value,data=test,color=Variable)

#calculate autocorrelation
test=test[order(test$Variable,test$Iteration),]

test$lag1=c(1,test$Value[1:19999])
test$lag10=c(rep(1,times=10),test$Value[1:19990])
test$lag50=c(rep(1,times=50),test$Value[1:19950])
test$lag100=c(rep(1,times=100),test$Value[1:19900])
cor.test(test$Value,test$lag1)
cor.test(test$Value,test$lag10)
cor.test(test$Value,test$lag50)
cor.test(test$Value,test$lag100)
