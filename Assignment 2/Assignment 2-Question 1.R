...
BiProbit=function(X,Y,N,mean,variance){
  ...
  return(list(draw,variancetheta))
}
...
N=10000
mean=c(10,10,10)
variance=diag(c(5,5,5))
test=BiProbit(X,Y,N,mean,variance)

#esimate
ThetaEstimate=test[[1]]
ThetaVariance=test[[2]]

colMeans(ThetaEstimate)
ThetaEstimate=as.data.frame(ThetaEstimate)
names(ThetaEstimate)=c('Theta1','Theta2','Theta3')
ThetaEstimate$Iteration=1:10000
ThetaEstimate=melt(ThetaEstimate,id.vars = 4)
names(ThetaEstimate)=c('Iteration','Variable','Value')
qplot(Iteration,Value,data=ThetaEstimate,color=Variable)
